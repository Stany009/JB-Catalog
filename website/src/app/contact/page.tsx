...
function ContactForm() {
  ...
  const buildWhatsAppMessage = () => {
    const lines = ['Hello JB Pools & Accessories,'];
    lines.push('');
    lines.push(`Name: ${form.name}`);
    if (form.phone) lines.push(`Phone: ${form.phone}`);
    if (form.email) lines.push(`Email: ${form.email}`);
    if (form.product) lines.push(`Product: ${form.product}`);
    lines.push('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const waUrl = `https://wa.me/${SITE_CONFIG.whatsappNumber}?text=${encodeURIComponent(buildWhatsAppMessage())}`;
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
        ...
        <p className="text-sm text-gray-500 mb-6">
          Thank you, <strong>{form.name}</strong>. Your enquiry has been sent via WhatsApp. We&apos;ll get back to you shortly.
          {form.product && (
            <>
